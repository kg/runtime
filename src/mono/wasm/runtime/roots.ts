...
export function mono_wasm_new_root_buffer(capacity: number, name?: string): WasmRootBuffer {
    ...
    return new WasmRootBuffer(<any>offset, capacity, WasmRootBufferType.Owned, name);
}
...
export function mono_wasm_new_root_buffer_from_pointer(offset: VoidPtr, capacity: number, name?: string): WasmRootBuffer {
    ...
    return new WasmRootBuffer(<any>offset, capacity, WasmRootBufferType.External, name);
}
...
export function mono_wasm_with_hazard_buffer<TFunc extends (buffer: WasmRootBuffer, ...userdata: any) => any>(
    ...
    try {
        ...
        if (!wrapper)
            wrapper = new WasmRootBuffer(<any>offset, num_elements, WasmRootBufferType.Stack);
        else
            wrapper._unsafe_change_address(<any>offset, num_elements);
        wrapper.clear();
