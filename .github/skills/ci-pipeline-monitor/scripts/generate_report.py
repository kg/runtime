...
class ReportGenerator:
    ...
    def _one_failure(self, out, fail):
        ...
        if fail["github_issue_number"]:
            ...
            gh_issue_command += f'comment "{issue_url}" '
        else:
            out.append("GitHub Issue: NEW — creating new issue")
            gh_issue_command += "create "
            creating_new_issue = True

        if fail["labels"]:
            if creating_new_issue:
                for label in fail["labels"].split(','):
                    gh_issue_command += f'--label "{label.strip()}" '
        ...
        out.append("")
        if creating_new_issue:
            gh_issue_command += f'--title "Test Failure: {test_name}" '
            gh_issue_command += f'--milestone "{fail['milestone']}" '
        ...
        temp_file.flush()
        temp_file.close()
        gh_issue_command += f'--body-file "{body_path}" '
        
        # Include the generated text in the report as well to satisfy the validator
        for line in out:
            original_out.append(line)

        original_out.append(">>>")

        print(gh_issue_command)
        os.system(gh_issue_command)
