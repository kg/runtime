...
class ReportGenerator:
    ...
    def _one_failure(self, out, fail):
        ...
        if fail["github_issue_number"]:
            ...
            gh_issue_command.append(f'comment "{issue_url}"')
        else:
        ...
        if fail["labels"]:
            if creating_new_issue:
                for label in fail["labels"].split(','):
                    gh_issue_command.append(f'--label "{label.strip()}"')
        ...
        out.append("")
        if creating_new_issue:
            gh_issue_command.append(f'--title "Test Failure: {test_name}"')
            gh_issue_command.append(f'--milestone "{fail['milestone']}"')
        ...
        temp_file.flush()
        temp_file.close()
        gh_issue_command.append(f'--body-file "{body_path}"')
